...
export class UsersService {
  ...
  private async getUserFlags(
    userId: string,
  ): Promise<{ is_premium: boolean; role: string | null }> {
  ...
  async getOrCreateUsage(userId: string) {
    ...
    if (createError) {
      throw new InternalServerErrorException(
        `Failed to create usage record: ${createError.message}`,
      );
    }
  ...
  async consumeDocument(userId: string) {
    ...
    if (error) {
      throw new InternalServerErrorException(
        `Failed to consume document allowance: ${error.message}`,
      );
    }
  }
  ...
  async consumeAiDiagnostic(userId: string) {
    ...
    if (error) {
      throw new InternalServerErrorException(
        `Failed to consume AI diagnostic allowance: ${error.message}`,
      );
    }
  }
  ...
  async consumeAiUpgrade(userId: string) {
    ...
    if (error) {
      throw new InternalServerErrorException(
        `Failed to consume AI upgrade allowance: ${error.message}`,
      );
    }
  }
